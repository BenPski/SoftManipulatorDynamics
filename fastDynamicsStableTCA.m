function [g,xi,eta] = fastDynamicsStableTCA(F,eta_prev,xi_prev,dt)
...
%     eta_guess = (4/3)*eta_prev-(1/3)*eta_prev_prev+(2/3)*dt*eta_der;
    vals = fsolve(@(x) newDynamicsHelper(@(eta) dynamicsStableTCA(F,eta,eta_prev,xi_prev,dt),x),[xi_prev(1,:);eta_guess(2:end,:)],options);
    [y,bcs] = dynamicsStableTCA(F,vals,eta_prev,xi_prev,dt);
